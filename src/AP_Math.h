...
#define constrain_ftype(amt, low, high) constrain_value(float(amt), float(low), float(high))
...
float norm(const T first, const U second, const P thr)
{
    return sqrtF(sq(first) + sq(second) + sq(thr));
}
